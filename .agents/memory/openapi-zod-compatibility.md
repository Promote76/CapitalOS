...
**How to apply:** Prefer explicit object properties or a schema shape that generates supported Zod primitives, then regenerate both the React client and server validators. Keep custom provider validation in the API server’s local boundary unless that package explicitly owns a direct Zod dependency.