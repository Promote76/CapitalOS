...
test("execution control persists, isolates households, audits denials, and serializes STOP", { skip: !enabled }, async () => {
  ...
  const [deniedAudit] = await db.select({ count: sql<number>`count(*)` }).from(auditEvents).where(and(
    eq(auditEvents.householdId, householdA.id),
    eq(auditEvents.eventType, "execution_control_transition_attempt"),
    sql`${auditEvents.metadata}->>'result' = 'DENIED_INVALID_TRANSITION'`,
  ));
  assert.ok(Number(deniedAudit?.count ?? 0) >= 1);
  const [attributedAudit] = await db.select({ count: sql<number>`count(*)` }).from(auditEvents).where(and(
    eq(auditEvents.householdId, householdA.id),
    eq(auditEvents.eventType, "execution_control_transition_attempt"),
    eq(auditEvents.actor, userA.id),
  ));
  assert.ok(Number(attributedAudit?.count ?? 0) >= 5);
  ...
  // Certification targets are disposable and audit history is append-only. Keep
  // the fixture rows so the retained evidence includes attribution and replay.
});