...
test("P0-01 preflight inventories all 134 routes and rejects unsafe generic probes", { skip: !enabled }, async () => {
  process.env.NODE_ENV = "test";
  process.env.CAPITAL_OS_TEST_CONTEXT = "1";
  process.env.CAPITAL_OS_ALLOWED_ORIGIN = "http://capitalos.test";
  const fixture = await createFixture();
  ...
  try {
    ...
    assert.equal(routes.length, 134, "The route inventory changed; update the certification matrix before running it.");
