...
import {
  GetFinancialDocumentParams, IngestFinancialDocumentBody, IngestFinancialDocumentResponse,
...
import { getFinancialDocument, ingestFinancialDocument, listFinancialDocuments, listFinancialReviewQueue, requestFinancialDocumentUploadUrl, reviewFinancialDocument } from "../services/financial-documents";
...
router.get("/financial-documents/:documentId", asyncRoute(async (req, res) => { const { documentId } = GetFinancialDocumentParams.parse(req.params); res.json(await getFinancialDocument(actorFrom(res), documentId)); }));
