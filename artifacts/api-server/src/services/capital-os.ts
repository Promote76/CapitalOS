...
export async function getProperty(actor: Actor) {
  ...
  return {
    ...
    targetBudget: String(property.targetBudget ?? "0.00"),
    estimatedDownPayment: String(property.estimatedDownPayment ?? "0.00"),
    estimatedClosingCosts: String(property.estimatedClosingCosts ?? "0.00"),
    readinessScore: Number(property.readinessScore),
    milestones: milestones.map((milestone) => ({
      id: milestone.id,
      name: milestone.name,
      status: milestone.status,
      progress: String(milestone.progress ?? "0.00"),
