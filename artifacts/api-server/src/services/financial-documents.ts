...
export async function retryBankStatementParser(actor: Actor, documentId: string, input: {
  ...
  return db.transaction(async (tx) => {
    ...
    await appendAuditEvent({
      ...
      entityId: documentId,
      reason: input.reason,
      metadata: { parserGenerationId: generation.id, parserVersion: BANK_STATEMENT_PARSER_VERSION, parserErrorKind: parsed.errorKind, parserErrors: parsed.errors, documentHash: document.documentHash, sourceObjectPreserved: true, previousReviewDecision: document.reviewDecision ?? null, priorReviewDecisionInvalidated: Boolean(document.reviewDecision) },
    }, tx);
    await tx.insert(idempotencyKeys).values({
      householdId: actor.householdId,
      key: input.idempotencyKey,
