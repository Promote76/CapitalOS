...
export async function getBudgetPlanningCategoryContributionDetail(actor: Actor, periodId: string, categoryId: string) {
  ...
  const includedNetActivity = included.reduce((sum, t) => sum + numeric(t.amount), 0);
  const includedActual = Math.max(0, snapshot.categoryType === "income" ? includedNetActivity : -includedNetActivity).toFixed(2);
