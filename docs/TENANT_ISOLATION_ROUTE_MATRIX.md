...
<!-- tenant-route-inventory: 213 -->
**Inventory result:** 213 Express route/method pairs match the executable route inventory.
...
The full 213-route preflight target is defined below. The latest clean guarded
...
- The route preflight discovers and asserts exactly 213 route/method pairs, compares household-scoped collection reads, sends same-household, foreign, and malformed path identifiers, injects mass-assignment fields into applicable generic write probes, and fails on unexpected server errors. The inventory includes the Document-to-Budget Bridge routes alongside the existing Financial Document Inbox, Business Income, Daily Ops, and Family Office routes.
