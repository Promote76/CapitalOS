...
<!-- tenant-route-inventory: 257 -->
**Inventory result:** 257 Express route/method pairs match the executable route inventory.
...
The full 257-route preflight target is the authoritative inventory. RC1 acceptance
...
| GET /research/opportunities; POST /research/opportunities/discover | Lens/search/refine fields only; no household, provider path, execution, account, transfer, allocation, or capital fields | Authenticated actor household supplies the known-symbol universe, Market Data connection, pending evidence drafts, approved evidence, portfolio observations, and deterministic rankings; provider-wide screener absence is explicit | PASS | PASS | PASS | PASS |
...
- The route inventory gate discovers and asserts exactly 257 route/method pairs and requires the exact-source RC1 certification to record that same count as executed. Historical route replays are not promoted to current evidence.
...
- The 2026-09-07 guarded replay remains historical evidence for its earlier surface. It is not evidence for the current 257-route inventory.
