...
<!-- tenant-route-inventory: 224 -->
**Inventory result:** 224 Express route/method pairs match the executable route inventory.
...
The full 224-route preflight target is defined below. The latest clean guarded
...
| GET /integrations/schwab/status; POST /integrations/schwab/connect, /refresh, /sync, /disconnect; GET /integrations/schwab/oauth/callback | OAuth callback state only; no household or actor query/body fields | Authenticated actor household for protected routes; callback uses atomic, expiring, single-use state bound to household and actor | PASS | PASS | PASS | PASS |
