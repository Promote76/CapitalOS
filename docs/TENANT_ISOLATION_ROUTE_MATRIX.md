...
**Inventory result:** 254 Express route/method pairs match the executable route inventory.
...
The full 254-route preflight target is the authoritative inventory. RC1 acceptance
...
- The route inventory gate discovers and asserts exactly 254 route/method pairs and requires the exact-source RC1 certification to record that same count as executed. Historical route replays are not promoted to current evidence.
...
- The 2026-09-07 guarded replay remains historical evidence for its earlier surface. It is not evidence for the current 254-route inventory.
