...
const gates = [
  ...
  ["Bank Statement Ingestion", "PASS", [["lib/db/src/schema/financial-documents.ts", /bankStatementDocuments/], ["artifacts/api-server/src/services/bank-statement-parser.ts", /BANK_STATEMENT_PARSER_VERSION = "bank-statement-v3"/], ["artifacts/api-server/src/services/bank-statement-parser.ts", /reconcileStatementSummary/], ["artifacts/api-server/src/integration/financial-document-ingest.test.ts", /Financial Inbox ingest persists Wells Fargo parser outcomes/], ["artifacts/api-server/package.json", /test:financial-document-ingest/], [".github/workflows/ci.yml", /Run Wells Fargo Financial Inbox integration/], ["artifacts/api-server/src/services/financial-documents.ts", /DOCUMENT_EVIDENCE_PENDING_REVIEW/]]],
